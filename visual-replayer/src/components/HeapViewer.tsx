                ...
                {dongles.map((dongle) => (
                    <div key={dongle.id} className='heap-item'>
                        <div className='heap-item-header'>
                            <span className='dongle-id'>
                                Dongle {dongle.id}
                            </span>
                            <span
                                className={`dongle-status ${dongle.current_owner_id !== null ? 'active' : ''}`}
                            >
                                {dongle.current_owner_id !== null
                                    ? `Owned by C${dongle.current_owner_id}`
                                    : 'Idle'}
                            </span>
                        </div>
                        {dongle.queue.length > 0 ? (
                            <div className='queue-viz'>
                                {dongle.queue.map((coderId, idx) => (
                                    <div
                                        key={`${dongle.id}-${coderId}-${idx}`}
                                        className='queue-node'
                                    >
                                        <span className='coder-badge'>
                                            C{coderId}
                                        </span>
                                        <span className='priority-tag'>
                                            P:{dongle.priorities[idx]}
                                        </span>
                                    </div>
                                ))}
                            </div>
